...
public class SalePaymentMovementConfiguration : IEntityTypeConfiguration<SalePaymentMovement>
{
    public void Configure(EntityTypeBuilder<SalePaymentMovement> builder)
    {
        ...
        builder.HasOne(x => x.ReversedSalePaymentMovement)
            .WithMany(x => x.ReversalMovements)
            .HasForeignKey(x => x.ReversedSalePaymentMovementId)
        ...
        builder.ToTable("sale_payment_movements", t =>
        {
            t.HasCheckConstraint(
                "ck_sale_payment_movements_gross_amount_positive",
                "gross_amount > 0");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_commission_percentage_non_negative",
                "commission_percentage >= 0");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_commission_amount_non_negative",
                "commission_amount >= 0");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_income_tax_percentage_non_negative",
                "income_tax_percentage >= 0");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_income_tax_amount_non_negative",
                "income_tax_amount >= 0");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_net_received_amount_non_negative",
                "net_received_amount >= 0");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_commission_not_greater_than_amount",
                "commission_amount + income_tax_amount <= gross_amount");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_net_received_amount_matches_components",
                "net_received_amount = gross_amount - commission_amount - income_tax_amount");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_in_does_not_reverse",
                $"movement_direction_id <> {(int)MovementDirectionOptions.In} OR reversed_sale_payment_movement_id IS NULL");
            t.HasCheckConstraint(
                "ck_sale_payment_movements_card_refund_reverses_original",
                $"movement_direction_id <> {(int)MovementDirectionOptions.Out} OR payment_method_id <> {(int)PaymentMethodOption.Card} OR reversed_sale_payment_movement_id IS NOT NULL");
        });
    }
}
