...
public class SaleDeliveryConfiguration : IEntityTypeConfiguration<SaleDelivery>
{
    public void Configure(EntityTypeBuilder<SaleDelivery> builder)
    {
        ...
        builder.Property(x => x.CollectionExchangeRate).HasPrecision(10, 4);
        ...
        builder.HasOne(x => x.DeliveryAgencyReconciliation).WithMany(x => x.Deliveries).HasForeignKey(x => x.DeliveryAgencyReconciliationId).OnDelete(DeleteBehavior.Restrict);
        ...
        builder.ToTable(t =>
        {
            ...
            t.HasCheckConstraint(
                "ck_sale_deliveries_change_given_nio_non_negative",
                "change_given_nio >= 0");
            t.HasCheckConstraint(
                "ck_sale_deliveries_collection_exchange_rate_required_for_usd",
                @"(
