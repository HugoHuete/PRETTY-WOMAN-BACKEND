...
`POST /api/v1/products/{productId}/images` recibe `multipart/form-data` con un campo `file`.
...
Para actualizar en una sola operación el orden y la imagen principal, usar `PUT /api/v1/products/{productId}/images`:
